...
vm_reservations = new Vue({
    ...
    methods: { 
        ...
        addVisitorRow: function() {
            ...
            orIcon.classList.add('fas','fa-user','prefix','grey-text')  
            var orInput = document.createElement('input');
            orInput.id ="vis-" + cnt;
            orInput.type = "text";
            orInput.classList.add('form-control', 'validate')
        ...
        checkReservationExists: function () {
            ...
            var vm = this
            let nonVisitors = vm.myReservations.filter(res =>{return res.visitor == null})
            let pos = nonVisitors.map(nvRes => {return nvRes.date}).indexOf(vm.selectedDate)
