...
$(document).ready(function () {
    ...
    $(".dropdown-menu a").click(function () {
        vm_reservations.reservationButtonClick(this)
    });
    ...
    document.getElementById('modalVisitorsForm').addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains("delVisitor")) {
            vm_reservations.deleteVisitorRow(e)
        }
    })
    ...
    $("#modalVisitorsForm").on("hidden.bs.modal", function () {
        ...
        $('#visitorCapacityMessage').innerHTML = '';
    });
...
vm_reservations = new Vue({
    ...
    methods: {
        ...
        addVisitorRow: function () {
            ...
            topDIV.classList.add('md-form', 'mb-2')
            ...
            delDiv.classList.add('delVisitor')
            var delIcon = document.createElement('i');
            delIcon.classList.add('far', 'fa-times-circle', 'fa-lg', 'delVisitor')

            topDIV.appendChild(orIcon);
            topDIV.appendChild(orInput);
            topDIV.appendChild(orlabel);
            delDiv.appendChild(delIcon);
            topDIV.appendChild(delDiv);
            document.getElementById('modalBodyVisitorsForm').appendChild(topDIV);

            this.setVisitorsCapacityText();

        },

        setVisitorsCapacityText: function () {

            var vm = this;

            $('.addVisitor').removeClass('hideElement');
            var cnt = document.getElementById('modalBodyVisitorsForm').childElementCount - 1

            console.log('cnt', cnt);
            console.log('selectedOfficeCapacity', vm.selectedOfficeCapacity);
            console.log('selectedOfficeCurrent', vm.selectedOfficeCurrent);

            if (vm.selectedOfficeCapacity - (cnt + vm.selectedOfficeCurrent) == 1) {
                $('.addVisitor').addClass('hideElement');
            }

            document.getElementById('visitorCapacityMessage').innerHTML = vm.selectedOfficeCapacity - (cnt + vm.selectedOfficeCurrent) + ' Place(s) Remaining';
        },

        deleteVisitorRow: function (e) {

            e.target.parentElement.parentElement.remove()
            this.setVisitorsCapacityText();
            //Relabel existing child elements
            document.getElementById('modalBodyVisitorsForm').children.forEach((elem, index) => {
                let cnt = index + 1
                let label = elem.children.item(2);
                label.innerHTML = "Visitor " + cnt;
                label.htmlFor = "vis-" + cnt;
                elem.id = "vis-" + cnt;
            })
        },
        ...
        refreshReservations: function () {
            ...
            return vm.getAllReservations().then((response) => {
                ...
                //Set the highlight date on the calendar
                vm.highlightDay()
        ...
        highlightDay: function () {
            ...
            console.log("Highlighting Day")
            var vm = this;

            // Remove all the highlighted entries first
            var paras = document.getElementsByClassName('fc-daygrid-bg-harness');
            while (paras[0]) {paras[0].parentNode.removeChild(paras[0]);}

            //Now add the new element that highlights the Cell
            var selectedDate = moment(vm.selectedDate, 'DD-MM-YY').format('YYYY-MM-DD');

            var selectedDateElem = $('[data-date="' + selectedDate + '"]').find('.fc-daygrid-day-bg')[0];
            var div1 = document.createElement('div');
            div1.classList.add('fc-daygrid-bg-harness');
            div1.style.left = '0px';
            div1.style.right = '0px';
            var div2 = document.createElement('div');
            div2.classList.add('fc-highlight');

            div1.appendChild(div2);
            selectedDateElem.appendChild(div1);
        },
        ...
        reservationButtonClick: function (param) {
        ...
        notifyMaximumCapacity: function () {
            ...
            if (vm.selectedOfficeAtCapacity && vm.notifyAtCapacityEmail != null) {
        ...
        processVisitorsForm: function (e) {
            ...
            //Check The selected vistors does not exceed the capacity
            totalVisitors = document.querySelector("#modalVisitorsForm .modal-dialog .modal-content .modal-body .visitorsForm").childElementCount
            if (totalVisitors + vm.selectedOfficeCurrent > vm.selectedOfficeCapacity) {
                console.log("Capacity Exceeded")
            } else {
                console.log("Capacity NOT Exceeded")
            }
            ...
            formEntries.forEach((elem) => {
                ...
                if (elem.children[1].value) {
                    ...
                    itemProperties['VisitorsName'] = vm.currentUsersVisitor.split(" ").length == 1 ? vm.currentUsersVisitor + " Visitor" : vm.currentUsersVisitor;
